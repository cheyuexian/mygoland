...
func GetClientIP(ctx context.Context) (string, error) {
	...
	//{
	//	p,o :=metadata.FromIncomingContext(ctx)
	//	fmt.Println("metadata ",p,o)
	//}
	//{
	//	p :=grpc.ServerTransportStreamFromContext(ctx)
	//	fmt.Println("metadata2  ",p)
	//
	//}
...
func (s *testServer) EmptyCall(ctx context.Context, in *testpb.Empty) (*testpb.Empty, error) {
	fmt.Println("EmptyCall ")
	s11,err1 := GetClientIP(ctx)
...
func TestOneBackend() {
	...
	if err != nil {
		log.Fatal("failed to start servers: %v", err)
	}
	defer test.cleanup()
	...
	if _, err := testc.EmptyCall(ctx, &testpb.Empty{}); err == nil || status.Code(err) != codes.DeadlineExceeded {
		log.Fatalf("EmptyCall() = _, %v, want _, DeadlineExceeded", err)
	}
	log.Println(err)
...
func TestBackendsRoundRobin() {
	...
	for si := 0; si < backendCount; si++ {
		...
		for i := 0; i < 1000; i++ {
			...
			fmt.Println(p.Addr.String(),"",test.addresses[si],"---",i," si ",si)
			if p.Addr.String() == test.addresses[si] {
				//fmt.Println(p.Addr.String(),"",test.addresses[si],"---")
				connected = true
				break
			}
...
func TestAddressesRemoved() {
	...
	log.Fatal("No RPC failed after removing all addresses, want RPC to fail with DeadlineExceeded")
}

func TestCloseWithPendingRPC() {
	...
	for i := 0; i < 3; i++ {
		...
		go func() {
			...
			if _, err := testc.EmptyCall(ctx, &testpb.Empty{}); status.Code(err) == codes.DeadlineExceeded {
				log.Fatal("RPC failed because of deadline after cc is closed; want error the client connection is closing")
			}
			cancel()
		}()
	}
...
func TestNewAddressWhileBlocking() {
	r, cleanup := manual.GenerateAndRegisterManualResolver()
	defer cleanup()
...
func TestOneServerDown() {
	...
	for si := 0; si < backendCount; si++ {
		...
		for i := 0; i < 1000; i++ {
			if _, err := testc.EmptyCall(context.Background(), &testpb.Empty{}, grpc.Peer(&p)); err != nil {
				log.Fatalf("EmptyCall() = _, %v, want _, <nil>", err)
			}
			if p.Addr.String() == test.addresses[si] {
				connected = true
				break
	...
	for i := 0; i < 3*backendCount; i++ {
		...
		if p.Addr.String() != test.addresses[i%backendCount] {
			log.Fatalf("Index %d: want peer %v, got peer %v", i, test.addresses[i%backendCount], p.Addr.String())
		}
	}
	...
	if targetSeen != 2 {
		log.Fatal("Failed to see server[backendCount-1] twice without seeing server[backendCount]")
	}
	for i := 0; i < 3*backendCount; i++ {
		if _, err := testc.EmptyCall(context.Background(), &testpb.Empty{}, grpc.Peer(&p)); err != nil {
			log.Fatalf("EmptyCall() = _, %v, want _, <nil>", err)
...
func TestAllServersDown() {
	...
	for si := 0; si < backendCount; si++ {
		...
		if !connected {
			log.Fatalf("Connection to %v was not up after more than 1 second", test.addresses[si])
		}
	}
	...
	log.Fatalf("Failfast RPCs didn't fail with Unavailable after all servers are stopped")
}
